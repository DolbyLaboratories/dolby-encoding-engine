...
The provided source code was built and tested using the Kakadu SDK version 7.9.
...
Prerequisites:
- The "kdu_v79R.lib" and "kdu_v79R.dll" libraries on Windows
- The "libkdu_v79R.so" library on Linux
...
2. Copy the .dll library with the plugin and "libkdu_v79R.dll" to the Dolby Encoding Engine installation folder.

Build instructions, Linux:
1. Set the KDUROOT environment variable pointing to the folder with prerequisites.
2. Build using make/j2k_dec_kakadu/linux_amd64_gnu/Makefile
    - make j2k_dec_kakadu_release.so
3. Copy the .so library with the plugin and "libkdu_v79R.so" to the Dolby Encoding Engine installation folder.
